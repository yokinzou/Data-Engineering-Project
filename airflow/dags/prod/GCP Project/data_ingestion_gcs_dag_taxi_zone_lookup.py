...
dataset_file_list = ["taxi_zone_lookup.csv"]            # 数据集文件名

# 使用上下文管理器创建DAG
with DAG(
    dag_id="data_ingestion_gcs_dag_taxi_zone_lookup",    
    ...
    for dataset_file_name in dataset_file_list:
        dataset_url = f"https://github.com/DataTalksClub/nyc-tlc-data/releases/download/misc/{dataset_file_name}"

        # 修改task_id，确保每个文件有唯一的task_id
        download_dataset_task = BashOperator(
            task_id=f"download_dataset_task_{dataset_file_name}",    # 添加文件名使task_id唯一
            bash_command=f"curl -sS -L --fail {dataset_url} > {path_to_local_home}/{dataset_file_name}"
        )
        ...
        if dataset_file_name == dataset_file_list[0]:  # 只在第一次循环时创建数据集
            ...
            # 设置任务依赖关系
            download_dataset_task >> format_to_parquet_task >> local_to_gcs_task >> create_dataset_task >> bigquery_external_table_task
        else:
            # 对其他文件，跳过创建数据集的步骤
